...
public class QuestionStock implements Serializable, Cloneable {
	/**
	 * 
	 */
	...
	public void cloneTest(Test test) throws CloneNotSupportedException {
		this.test = test.clone();
	}
	
	public void cloneTestFromVector(Vector<Question> questions) throws CloneNotSupportedException {
		cloneTest(new Test(questions));
	}
	...
	public boolean setOpenAnswer(String ansText, int id) throws CloneNotSupportedException {
		int index = getIndexByID(id);
		if (index == -1)
			return false;
	...
	public boolean setQuestion(String queText, int id) {
		...
		if (questions.get(index) instanceof OpenQuestion) {
			...
			que.setSerialNumber(que.getSerialNumber()-1);
			if (isExists(que))
				return false;
		} else {
	...
	public void saveQuestions (PrintWriter pw) {
		for (int i = 0; i < questions.size(); i++) {
			pw.print(i+1 + ") ");
	...
	public void saveAnswers(PrintWriter pw) {
		for (int i = 0; i < questions.size(); i++) {
			pw.print(i + 1 + ") ");
			if(questions.get(i) instanceof OpenQuestion) {
				OpenQuestion oq = (OpenQuestion) questions.get(i);
				oq.saveAnswer(pw);				
			} else {
				pw.println();
				AmericanQuestion aq = (AmericanQuestion) questions.get(i);
				aq.saveAnswer(pw);		
			}
		}		
	}
	...
	@Override
	public QuestionStock clone() throws CloneNotSupportedException {
		QuestionStock tmp = (QuestionStock)super.clone();
		tmp.test = this.test.clone();
		for (int i = 0; i < questions.size(); i++) {
			if(questions.get(i) instanceof OpenQuestion) {
				tmp.getQuestions().set(i, (OpenQuestion)this.questions.get(i).clone());
			} else {
				tmp.getQuestions().set(i, (AmericanQuestion)this.questions.get(i).clone());
			}		
		}
		return tmp;
	}
