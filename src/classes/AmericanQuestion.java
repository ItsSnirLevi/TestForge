...
public class AmericanQuestion extends Question implements Serializable {
	...
	public void saveAnswer(PrintWriter pw) {
		...
		for (int i = 0; i < answers.size(); i++) {
			pw.print("  " + (c++) + ") ");
			pw.println(answers.get(i).getText() + ((answers.get(i).isCorrect())?"\tCorrect":"\tWrong"));
		}
	}
	
	public void saveQuestionOptions(PrintWriter pw) {
		char c = 'a';
		for (int i = 0; i < answers.size(); i++) {
			pw.print("  " + (c++) + ") ");
			pw.println(answers.get(i).getText());
		}
	}
	...
	public AmericanQuestion clone() throws CloneNotSupportedException {
		...
		for (int i = 0; i < answers.size(); i++) {
			tmp.getAnswers().set(i, this.answers.get(i).clone());
		}
		return tmp;
	}
